(() => {
  ...
  function showOverlay(title, buttonText) {
    ...
    if (touchUI) touchUI.style.display = "none"; // KEY FIX
  }

  function hideOverlay() {
    if (overlay) overlay.style.display = "none";
    if (touchUI) touchUI.style.display = "flex"; // KEY FIX
  }

  // ===== iPhone: stop page scroll stealing input =====
  if (canvas) {
  ...
  function bindTap(el, key) {
    ...
    const tap = (ev) => {
      ev.preventDefault();
      setKey(key, true);
      setTimeout(() => setKey(key, false), 80);
    };
  ...
  bindHold(btnRight, "d");
  bindTap(btnJump, "w");
  bindHold(btnShoot, "j"); // hold to shoot
  bindTap(btnGren, "k");
  bindTap(btnDash, "l");
  bindTap(btnSlide, "s");

  // ===== Canvas scaling (iPhone-safe) =====
  const BASE_W = 960;
  const BASE_H = 540;
  const W = BASE_W;
  const H = BASE_H;
  ...
  let bullets = [];   // {x,y,vx,vy,r,dmg,hostile}
  let grenades = [];  // {x,y,vx,vy,r,t,fuse,dmg,blast}
  let particles = []; // {x,y,vx,vy,life,kind}
  let enemies = [];   // {type,x,y,vx,vy,w,h,hp,dmg,score, ...}
  let pickups = [];   // {type,x,y,vy,t}
  let boss = null;    // {x,y,vx,w,h,hp,maxhp,atkCD,dmg}
  ...
  function syncHUD() {
    if (hpText) hpText.textContent = String(Math.max(0, Math.floor(player.hp)));
    if (ammoText) ammoText.textContent = "∞";
    if (grenText) grenText.textContent = String(player.gren);
    if (scoreText) scoreText.textContent = String(score);
    if (waveText) waveText.textContent = String(wave);
  }
  ...
  function waveLogic(dt) {
    ...
    while (spawnBudget > 900) {
      ...
      const r = Math.random();
      if (r < 0.46) spawnRaider();
      else if (r < 0.83) spawnDrone();
      else spawnShield();
    }
  ...
  function update(dt) {
    ...
    bullets = bullets.filter((b) => b.x > -140 && b.x < W + 140);
    ...
    for (const g of grenades) {
      ...
      if (g.y > groundY - 6) {
        ...
        g.vx *= 0.76;
        for (let i = 0; i < 4; i++) {
          particles.push({ x: g.x, y: g.y, vx: rand(-160, 160), vy: rand(-160, 20), life: rand(90, 150), kind: "spark" });
        }
      }
    ...
    for (const b of bullets) {
      ...
      if (circleAabb(b.x, b.y, b.r, player.x - player.w / 2, player.y - player.h, player.w, player.h)) {
        b._dead = true;
        hurt(b.dmg);
        for (let i = 0; i < 10; i++) particles.push({ x: b.x, y: b.y, vx: rand(-240, 240), vy: rand(-240, 240), life: rand(80, 140), kind: "spark" });
      }
    }
    ...
    for (const b of bullets) {
      ...
      if (boss && circleAabb(b.x, b.y, b.r, boss.x - boss.w / 2, boss.y - boss.h, boss.w, boss.h)) {
        ...
        for (let i = 0; i < 8; i++) particles.push({ x: b.x, y: b.y, vx: rand(-200, 200), vy: rand(-200, 200), life: rand(70, 120), kind: "spark" });
      }
      ...
      for (const e of enemies) {
        if (circleAabb(b.x, b.y, b.r, e.x - e.w / 2, e.y - e.h, e.w, e.h)) {
          ...
          for (let i = 0; i < 10; i++) particles.push({ x: b.x, y: b.y, vx: rand(-240, 240), vy: rand(-240, 240), life: rand(80, 150), kind: "spark" });
          break;
        }
      }
    }
  ...
  function restart() {
    ...
    running = false;
    paused = false;
    showOverlay("Touch Controls (iPhone)", "Start");
    showToast("Restarted");
  }
  ...
  // ===== Init =====
  resetAll();
  showOverlay("Touch Controls (iPhone)", "Start"); // hide touch UI on menu
  requestAnimationFrame(loop);
})();